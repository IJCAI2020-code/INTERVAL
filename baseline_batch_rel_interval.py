    ...
    def collate_fn(self, batch):
        ...
        for i, sample in enumerate(batch):
            if 'r' not in sample.keys():
                if self.ratio is not None and not np.isnan(self.ratio):
                    res = extract_r_x(
                        sample['x'][:lengths[i]], self.hparams.has_value)
                elif len(sample['x']) <= truncation:
                    res = extract_r_x(
                        sample['x'], self.hparams.has_value)
                else:
                    res = extract_r_x(
                        sample['x'][-(truncation+self.truncation_offset):-self.truncation_offset], self.hparams.has_value)

                sample['x'] = res[0]
                sample['r'] = res[1]

                if self.hparams.has_value:
                    sample['v'] = res[2]
            else:
            ...
            if self.hparams.has_value:
                Va.append(sample['v'])
        if self.hparams.has_value:
            return torch.LongTensor(X), torch.LongTensor(Y), torch.stack(R, dim=0), torch.LongTensor(lengths)-1, torch.LongTensor(Va)
        else:
...
class MyModel(pl.LightningModule):
    ...
    def add_model_specific_args(parent_parser, root_dir):
        ...
        parser.add_argument('--has_value', default=False, action='store_true')
        parser.add_argument('--is_multi', default=True, action='store_true')
        parser.add_argument('--truncation', default=128, type=int)
        ...
        parser.opt_list('--batch_size', default=2, type=int)
        return parser
...
def run_train():
    ...
    demo_log_dir = os.path.join(root_dir, 'day_rel_interval')
...
def run_test():
    ...
    #    trainer.test(model)
    model = MyModel.load_from_metrics(
        weights_path='saved_seed1/rel_interval_layer_varient/model_weights/ad/0/_ckpt_epoch_8.ckpt',
        tags_csv='saved_seed1/rel_interval_layer_varient/test_tube_data/ad/version_0/meta_tags.csv',
        on_gpu=True,
        map_location=torch.device('cuda')
    )
    trainer = Trainer()
